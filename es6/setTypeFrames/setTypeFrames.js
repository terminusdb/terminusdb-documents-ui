...
function makeSetSubChoiceTypeFrames(frame, item, uiFrame, mode, formData) {
  var properties = {},
      propertiesUI = {},
      layout = {},
      uiLayout = {};
  ...
  if (mode === _constants.CREATE) {
    layout = (0, _setType.getCreateSetSubChoiceDocumentTypeLayout)(frame, item);
    uiLayout = (0, _setType.getCreateSetSubChoiceDocumentTypeUILayout)(frame, item, uiFrame);
  }

  if (mode === _constants.EDIT) {
    layout = (0, _setType.getEditSetChoiceSubDocumentTypeLayout)(frame, item, formData);
    uiLayout = (0, _setType.getEditSetChoiceSubDocumentTypeUILayout)(frame, item);
  }

  if (mode === _constants.VIEW) {
    layout = (0, _setType.getViewSetChoiceSubDocumentTypeLayout)(frame, item, formData);
...
var makeSetTypeFrames = function makeSetTypeFrames(frame, item, uiFrame, mode, formData, onTraverse, onSelect, fullFrame, documentation) {
  ...
  if (frame.hasOwnProperty("properties") && frame["properties"].hasOwnProperty(item)) {
    if (frame["properties"][item].hasOwnProperty("info") && frame["properties"][item]["info"] === _constants.CHOICESUBCLASSES) madeFrames = makeSetSubChoiceTypeFrames(frame, item, uiFrame, mode, formData);
  } // set Choice Document classes
