import {ONEOFVALUES, CREATE, COORDINATES, CHOICECLASSES, CHOICESUBCLASSES, POINT_TYPE, SYS_UNIT_DATA_TYPE} from "./constants"
...
function checkIfChoiceTypeData(mode, schema, data, frame, current, type) {
    ...
    for(var item in schema.properties) {
		if(schema.properties[item].hasOwnProperty("info") && schema.properties[item]["info"] === CHOICESUBCLASSES) {
...
function modifyChoiceDocuments (mode, schema, data, frame, current, type) {
    ...
    if(!Object.keys(data).length) return false
    for(var item in schema.properties) {
		if(schema.properties[item].items.hasOwnProperty("info") && schema.properties[item].items["info"] === CHOICESUBCLASSES) {
            if(Array.isArray(modifiedData[item])){ //set
...
export const transformData_working = (mode, schema, data, frame, current, type, itemSchema) => {
	var extracted={}
	//let currentFrame=frame[current]
    let formData = data
            ...
            formData[key].map(fd => {
                ...
                if(typeof fd === "string") { // set of document classes
                    transformed=fd
                     ...
                     transformed=transformData(mode, schema, fd, frame, current, type)
                }
                if(transformed && Object.keys(transformed).length)
                    transformedArray.push(transformed)
            })
