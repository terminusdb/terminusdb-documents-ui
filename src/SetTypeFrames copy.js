...
function getAnyOfProperties(formData, setFrames, item) {
    ...
    setFrames.properties[item].properties["@oneOf"]["anyOf"].map(aOf => {
        anyOfPropertyArray.push(aOf.properties)
    })
...
export function makeSetSubDocuments (setFrames, item, uiFrame, mode, formData, onTraverse) {
    ...
    if(mode !== CREATE && formData.hasOwnProperty(item)){
        ...
        if(formData.hasOwnProperty(item)) {
            ...
            var tempFormData = formData[item]

            for(var x=0; x<defaultValues.length; x++){
                if(subProperties.hasOwnProperty(ONEOFVALUES)) { //@oneOf (used in seshat)
                    layout["info"] = ONEOFVALUES
                    let oneOfProperties = getAnyOfProperties(formData, setFrames, item)


                    for(var oneOf=0; oneOf < oneOfProperties.length; oneOf++){ // one of properties
                        let choice = Object.keys(oneOfProperties[oneOf])[0]

                        if(tempFormData[x].hasOwnProperty(choice)) { // match found

                            /*filledItems.push({
                                type: "object",
                                properties: oneOfProperties[oneOf][choice].properties,
                                default: tempFormData[x][choice]
                            }) */
                            ...
                            //console.log("oneOfProperties", oneOfProperties)
    ...
    // add different property ui for @oneOfs
    /*if(mode === EDIT && properties[item].hasOwnProperty("info") && properties[item].info === ONEOFVALUES){


        let propUI = propertiesUI[item]["items"]["@oneOf"]

        for(var key in propUI) {
            propertiesUI[item]["items"][key] = propUI[key]
        }
